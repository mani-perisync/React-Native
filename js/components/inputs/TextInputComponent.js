...
import { borderColor, borderWidth, flex, fontSize, marginPosition, opacity, padding, paddingPoistion, radius, styles, textColor, widthValue } from "../../styles/Styles";
import { IconComponent } from "../view";

export const TextInputCompnent = ({secureTextEntry,widthValue, placeholder,placeholderTextColor, value, onChangeText, editable, onKeyPress, maxLength, keyboardType, enableClear, clearPressed, style,size }) => {
        ...
        <View style={[styles.row, padding(10), styles.centerHorizontal, borderWidth(0.5), radius(6), borderColor(Colors.blue),style]}>
            <View style={[styles.row, flex(1.5), value == "" && opacity(0.7), styles.centerHorizontal ]}>
                <TextInput
                    ...
                    style={[ textColor(darkMode), fontSize(size?size:18), flex(1), padding(0, 6, 12),{width:widthValue}]}
                />
